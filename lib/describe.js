...
//NEED TO IMPLEMENT A FILTER FUNCTION FOR CHILDREN!!!

  //uses match module to sweep d block for open it tags
  myDescribe.openChildren = matches.matches(myDescribe.origin, myDescribe.end, iOpen, file).filter(function(value){
  ...
  myDescribe.closeChildren = matches.matches(myDescribe.origin, myDescribe.end, iClose, file).filter(function(value){
      if(value >= myDescribe.globalOrigin && value <= myDescribe.globalEnd){
        return value;
      }
  });
